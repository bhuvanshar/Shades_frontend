...
import { createAddress, deleteAddress, forgotPassword, getAddresses, getMyOrders, setDefaultAddress, updateAddress, updateCurrentUser, getCommunicationPreferences, updateCommunicationPreferences } from "../../services/api";
...
export default function Account() {
  ...
  const resetPassword = async () => { setSaving(true); try { const result = await forgotPassword(user.email); setNotice(result.message || "Password reset email sent."); } catch(e) { setError(e.message); } finally { setSaving(false); } };
  ...
  return <main className="account-page"><div className="container">
    ...
    <div className="account-layout"><section className="account-panel"><div className="account-title"><span>Personal information</span><h2>Your profile</h2></div><form className="profile-form" onSubmit={saveProfile}><label>Full name<input value={profile.name} onChange={(e) => setProfile({...profile,name:e.target.value})} required /></label><label>Email address<input value={user.email} disabled /><small>Email cannot be changed here for account security.</small></label><label>Phone number<input value={profile.phoneNumber} maxLength="20" onChange={(e) => setProfile({...profile,phoneNumber:e.target.value})} /></label><button disabled={saving}>Save profile</button></form></section>
