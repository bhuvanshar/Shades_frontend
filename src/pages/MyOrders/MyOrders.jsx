...
export default function MyOrders() {
  ...
  const cancelCustomerOrder = async (order) => {
    const paid = order.payments?.some((payment) => ["PAID", "PARTIALLY_REFUNDED"].includes(payment.status));
    const detail = paid ? " The remaining paid balance will be recorded for refund." : "";
    if (!window.confirm(`Cancel order #${order.orderId}?${detail} This cannot be undone.`)) return;
    setCancellingOrder(order.orderId); setError(""); setNotice("");
    try {
  ...
  return <main className="my-orders-page"><div className="container">
    ...
    {loading ? <div className="orders-empty">Loading your orders…</div> : orders.length === 0 ? <div className="orders-empty"><span>SW</span><h2>No orders yet</h2><p>Your Shades World purchases will appear here.</p><Link to="/#shop">Explore the collection</Link></div> : <section className="orders-list">{orders.map((order) => {
      ...
      return <article className="customer-order" key={order.orderId}>
        ...
        <div className="order-buttons"><button className="order-details-toggle" onClick={() => setExpanded(open ? null : order.orderId)}>{open ? "Hide details" : "View order details"}</button>{order.payments?.some((payment) => ["PAID", "PARTIALLY_REFUNDED", "REFUNDED"].includes(payment.status)) && <button className="invoice-download-button" disabled={downloadingInvoice === order.orderId} onClick={() => getInvoice(order.orderId)}>{downloadingInvoice === order.orderId ? "Preparing invoice…" : "Download invoice"}</button>}{["PLACED", "CONFIRMED"].includes(order.orderStatus) && <button className="cancel-order-button" disabled={cancellingOrder === order.orderId} onClick={() => cancelCustomerOrder(order)}>{cancellingOrder === order.orderId ? "Cancelling…" : "Cancel order"}</button>}{order.orderStatus === "DELIVERED" && withinReturnWindow && !active && <button className="request-return-button" onClick={() => openReturn(order)}>Request a return</button>}{order.orderStatus === "DELIVERED" && !withinReturnWindow && <small className="return-window-closed">Return window closed</small>}</div>
